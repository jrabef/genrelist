		...
		if ($dbSuccess) {
			...
			if (isset($_GET["tag"]))
			{
				if (!empty ($_GET["tag"]))
				{
					...
					if (!empty($_GET["userid"]))
					{
						...
						$bandids = mysql_query('select bandid, count(*) as tagcount from bandtag where name like "'.$tag.'" and creatorid = '.$userid.' group by bandid order by tagcount desc');
					...
					else 
					{
						...
						$bandids = mysql_query('select bandid, count(*) as tagcount from bandtag where name like "'.$tag.'" group by bandid order by tagcount desc');
			...
			{	
				...
				if ($result != false) {
					...
					$tags = mysql_query("select name, count(*) as tagcount from bandtag group by name");
